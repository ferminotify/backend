...
router.put('/register', async (req, res) => {
    ...
    try {
        ...
        try {
            const confirm_link = `${API_URL}/auth/register/confirmation/${telegramTemporaryCode}`;
            await sendMail(
                to = email,
                ...
                plainText = `Ciao ${name}! Per completare la registrazione, conferma il tuo indirizzo email: ${confirm_link}. Appena completerai la registrazione, ti arriverà una seconda email con tutte le indicazioni sull'utilizzo. A presto!`,
                headers = {
            ...
            pool.query(`DELETE FROM subscribers WHERE id = $1`, [result.rows[0].id], (err) => {
                if (err){
                    console.error("Failed to delete user after email send failure:", err);
                }
            });
            return res.status(500).json({ error: 'Errore interno!' });
        }
...
router.get('/register/confirmation/:code', async (req, res) => {
    ...
    try {
        const email = await getUserEmailWithTelegramID(userId);
        ...
        try{
            await sendMail(
                ...
                plainText = `Ciao ${name}! Benvenuto a Fermi Notify! Esplora la Dashboard a ${URL}/dashboard per personalizzare le notifiche e visita ${URL}/faq per scoprire come funziona Fermi Notify.`,
                headers = {
